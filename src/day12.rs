...
mod tests {
    ...
    fn it_works_on_the_example() -> Result<(), Box<dyn Error>> {
        let input = include_str!("../puzzles/day12/example.input")
            .lines()
            .map(String::from);

        let solver = Day12::try_create(Box::new(input)).unwrap();
    ...
    fn it_works_on_my_input() -> Result<(), Box<dyn Error>> {
        let input = include_str!("../puzzles/day12/joshua.input")
            .lines()
            .map(String::from);
    ...
    fn bench_parse(b: &mut Bencher) {
        b.iter(|| {
            ...
            let _solver = Day12::try_create(Box::new(input)).unwrap();
        });
    }
